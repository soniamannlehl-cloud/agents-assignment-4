...
# All 15 tools exposed by the MCP server (see mcp_server/app.py).
ALL_MCP_TOOL_NAMES = [
    "get_customer",
    "list_customers",
    "add_customer",
...
# Admin/destructive tools excluded from the Support Agent toolset.
ADMIN_TOOL_NAMES = [
    "disable_customer",
    "activate_customer",
    "delete_ticket",
...
# Support-safe tools: all MCP tools except admin operations.
SUPPORT_SAFE_TOOL_NAMES = [
    name for name in ALL_MCP_TOOL_NAMES if name not in ADMIN_TOOL_NAMES
]
...
def create_customer_data_toolset() -> McpToolset:
    ...
    return McpToolset(
        connection_params=SseConnectionParams(url=MCP_SSE_URL),
        tool_filter=ALL_MCP_TOOL_NAMES,
    )
...
def create_support_toolset() -> McpToolset:
    ...
    return McpToolset(
        connection_params=SseConnectionParams(url=MCP_SSE_URL),
        tool_filter=SUPPORT_SAFE_TOOL_NAMES,
    )
